...
describe("PiggyBankFactoryContract", function () {
  ...
  describe("createPiggyBank", function () {
      it("should fail if allowedTokens length is not 3", async function () {
   ...
   describe("withdrawPenalties", function () {
    it("Should fail if balance is zero", async function () {

      const { deployPiggyBank, tokenAAddress } = await loadFixture(deployPiggyBankFixture);

      await expect(deployPiggyBank.withdrawPenalties(tokenAAddress)).to.be.revertedWithCustomError(deployPiggyBank, "InsufficientBalance");
    });

    it("Should allow only owner to withdraw penalties", async function () {

      const { deployPiggyBank, tokenAAddress, addr1 } = await loadFixture(deployPiggyBankFixture);

      await expect(deployPiggyBank.connect(addr1).withdrawPenalties(tokenAAddress)).to.be.revertedWithCustomError(deployPiggyBank, "OwnableUnauthorizedAccount");
    });

    it("Should allow only owner to withdraw penalties successfully", async function () {

      const { deployPiggyBank, tokenA, tokenAAddress, owner, FactoryAddress } = await loadFixture(deployPiggyBankFixture);

      // Transfer some tokens to the factory to simulate penalties
      const penaltyAmount = ethers.parseUnits("100", 18);
      await tokenA.connect(owner).transfer(FactoryAddress, penaltyAmount);

      await expect(deployPiggyBank.connect(owner).withdrawPenalties(tokenAAddress)).to.emit(deployPiggyBank, "PenaltyWithdrawn").withArgs(tokenAAddress, tokenA.balanceOf(FactoryAddress));
    });
